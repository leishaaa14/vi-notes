...
function firePaste(el: HTMLElement, text: string): void {
...
describe("createPasteDetector", () => {
  ...
  it("calls onPaste callback for each event", () => {
    ...
    expect((cb.mock.calls[0][0] as PasteEvent).pastedLength).toBe(4);
  });
  ...
  it("stores an ISO timestamp", () => {
    ...
    expect(new Date(detector.events[0].timestamp).toISOString()).toBe(
      detector.events[0].timestamp
    );
  });
});
